...
public class WebRegisterActivity extends Activity {
	...
	private EditText mDomain, mWebAcc, mWebPwd, mName;
	...
	private class RequestTask extends AsyncTask<Void, Void, SignUpResult> implements OnCancelListener {
		...
		protected void onPreExecute() {
			mProgress = new ProgressDialog(WebRegisterActivity.this);
		...
		protected void onPostExecute(SignUpResult result) {
			...
			// occur EACException
			if (result == null) {				
				Toast.makeText(WebRegisterActivity.this, "Register Error", Toast.LENGTH_SHORT).show();
			
			// get response
			} else {
				int state = result.getState();
				if (state == 200) {
					Toast.makeText(WebRegisterActivity.this, "Register Success", Toast.LENGTH_SHORT).show();
					String domain = mDomain.getText().toString();
					String account = mWebAcc.getText().toString();
					String password = mWebPwd.getText().toString();
					AppUtils.saveWebLoginParams(WebRegisterActivity.this, domain, account, password, null);
					finish();
					
				} else {
					String msg = "Register Error\n";
					msg += result.getDescription() + "(" + state + ")";
					Toast.makeText(WebRegisterActivity.this, msg, Toast.LENGTH_SHORT).show();	
				}
			}
		...
		protected void onCancelled() {
			mProgress.dismiss();
			mRequestTask = null;
			Toast.makeText(WebRegisterActivity.this, "Cancelled", Toast.LENGTH_SHORT).show();
		}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// initialize view
		mDomain = (EditText) findViewById(R.id.reg_domain);
		mWebAcc = (EditText) findViewById(R.id.reg_acc);
		mWebPwd = (EditText) findViewById(R.id.reg_pwd);
	...
	protected void onResume() {
		...
		String domain = getString(R.string.def_reg_domain);
